...
| `range` | `tuple[float, float]` | `None` | Lower and upper range of the bins. Values outside are ignored. |
| `max_bins` | `int` | `None` | Maximum number of bins. If not provided, the optimal number is determined automatically. |
| `density` | `bool` | `False` | If `False`, return cumulative counts; if `True`, return cumulative probabilities. |

#### Returns

| Return | Type | Description |
|--------|------|-------------|
| `cumcount` | `NDArray[np.floating]` | The cumulative histogram values. |
| `bin_edges` | `NDArray[np.floating]` | Array of length `len(cumcount) + 1` containing the bin edges. |

#### See Also

- [`scipy.stats.cumfreq`](https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.cumfreq.html) — SciPy's cumulative histogram function.
- [`khisto.histogram`](#histogram) — Non-cumulative optimal histogram.

#### Examples

Basic cumulative counts:
